...
function find(int $id) : array { 
    ...
    function edit(array $produto) : bool {
        $conn = get_connection();
        $sql = 'UPDATE produto SET titulo = ?, descricao = ?, preco = ?, imagem = ? WHERE id = ?';
        $stmt = $conn->prepare($sql);
        $stmt-> bind_param(
            "ssdis",
            $produto['titulo'],
...
function deleteAvaliacaoByIdProduto(int $id): bool {        
    $conn = get_connection();
    $sql = 'DELETE FROM avaliacao_produto WHERE id_produto = ?';    
    $stmt = $conn->prepare($sql);    
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $success = $stmt->affected_rows > 0;
...
function editImagemById(string $url_imagem, int $id): bool{
        $conn = get_connection();
        $sql = 'UPDATE produto SET url_imagem = ? WHERE id = ?';
        $stmt = $conn->prepare($sql);
